...
class SAFE_handler:
        ...
        for root, _, files in os.walk(zip_folder):
            for file in files:
                if file.endswith('.zip'):
                    ...
                    os.remove(file_path)  # Deletes the zip file after extraction

                    # Create a .done file with the same name as the output dir
                    done_file_path = f'{extract_path}.done'
    ...
    def process_dir(safe_dir, date_dir):
        ...
        for done_file in done_files:
            ...
            tile_id = f"{safe_file.split('_')[5]}" #tile_id section of std sentinel-2 safe filename
            ...
            os.rename(done_file, processed_file)
